...
blur_mode = 'Blur'  # Chế độ làm mờ mặc định
...
def apply_blur_effect():
    if captured_frame is None:
        ...
        apply_filter(blur_mode, captured_frame)
...
def toggle_blur_mode():
    global blur_mode
    blur_mode = 'Blur' if blur_mode == 'Smooth' else 'Smooth'
    btn_blur.config(text=f"Apply {blur_mode} Effect")
...
def end_rect_selection(event):
    ...
    if start_point and end_point:
        remove_background_with_selection()
...
def remove_background_with_selection():
    """Xóa phông nền bằng cách giữ lại đối tượng được chọn bằng hình chữ nhật"""
    global captured_frame, start_point, end_point
    if captured_frame is not None and start_point and end_point:
        x1, y1 = start_point
        x2, y2 = end_point

        # Tạo mặt nạ ban đầu với toàn bộ ảnh là nền đen
        mask = np.zeros(captured_frame.shape[:2], dtype=np.uint8)

        # Thiết lập vùng hình chữ nhật là màu trắng trong mặt nạ
        mask[y1:y2, x1:x2] = 255

        # Áp dụng mặt nạ lên ảnh gốc để chỉ giữ lại vùng bên trong hình chữ nhật
        result = cv2.bitwise_and(captured_frame, captured_frame, mask=mask)

        # Phần nền bên ngoài hình chữ nhật sẽ được làm mờ
        blurred_frame = cv2.GaussianBlur(captured_frame, (21, 21), 0)
        final_image = np.where(mask[:, :, np.newaxis] == 255, result, blurred_frame)
...
def save_image():
    ...
    if captured_frame is None:
        ...
        if file_path:
            ...
            messagebox.showinfo("Image Saved", f"Image saved successfully to {file_path}")
...
# Nút xóa phông nền
btn_remove_bg = tk.Button(root, text="Remove Background", command=remove_background_with_selection)
btn_remove_bg.pack()
